...
when = set()
for order in [
		a32_order, t16_order, t32_order, a64_order, j32_order,
		cdp_order, ldc_order, mcr_order, mrc_order, mcrr_order, mrrc_order]:
	for _, ins in order.items():
		pair = (ins.get('added', ''), ins.get('removed', ''))
		if pair not in pairs:
			pairs[pair] = set()
		pairs[pair].add(ins['#'])
		for ex in ins.get('exclude', ()):
			for _set, kwd in [(_except, '\texcept\t'), (when, '\twhen\t')]:
				if kwd in ex:
					_set.add(ex.split(kwd)[1])

for pair, line in sorted(pairs.items(), key = lambda pair: pair[0]):
	#print(pair, line)
	print(pair)

print('except', sorted(_except))
print('when', sorted(when))

exit()

def get_full_mask(mode, order, full_mask1 = 0, matcher1 = 0, full_mask2 = 0, matcher2 = 0):
	submask2 = None
	if mode in {'a32', 'a64'}:
		submask1 = 0xFFFFFFFF
	elif mode in {'t16', 't32'}:
		submask1 = 0xFFFF
		if mode == 't32':
			submask2 = 0xFFFF

	for ins in order.values():
		_, mask1, value1, mask2, value2 = extract_mask(mode, ins['code'])
		if (value1 & full_mask1) == matcher1:
			submask1 &= mask1
		if mode == 't32' and (value2 & full_mask2) == matcher2:
			submask2 &= mask2

	return submask1, submask2

#a32_mask = 0xFFFFFFFF
#for ins in a32_order.values():
#	_, mask, value, _, _ = extract_mask('a32', ins['code'])
#	a32_mask &= mask
a32_mask, _ = get_full_mask('a32', a32_order)
print(f"A32: {a32_mask:032b}")
# ----##-...

matcher = 0
while True:
	print(f"-  {matcher:032b}")
	submask, _ = get_full_mask('a32', a32_order, a32_mask, matcher)
	print(f"=> {submask:032b}")
	matcher = ((matcher + 1) + ~a32_mask) & a32_mask
	if matcher == 0:
		break

#a64_mask = 0xFFFFFFFF
#for ins in a64_order.values():
#	_, mask, value, _, _ = extract_mask('a64', ins['code'])
#	a64_mask &= mask
a64_mask, _ = get_full_mask('a64', a64_order)
print(f"A64: {a64_mask:032b}")
# ---###-...

#t16_mask = 0xFFFF
#for ins in t16_order.values():
#	_, mask, value, _, _ = extract_mask('t16', ins['code'])
#	t16_mask &= mask
t16_mask, _ = get_full_mask('t16', t16_order)
print(f"T16: {t16_mask:016b}")
# ###-...

#t32_mask1 = t32_mask2 = 0xFFFF
#for ins in t32_order.values():
#	_, mask1, value1, mask2, value2 = extract_mask('t32', ins['code'])
#	t32_mask1 &= mask1
#	t32_mask2 &= mask2
t32_mask1, t32_mask2 = get_full_mask('t32', t32_order)
print(f"T32: {t32_mask1:016b} {t32_mask2:016b}")
# ###-#-...
